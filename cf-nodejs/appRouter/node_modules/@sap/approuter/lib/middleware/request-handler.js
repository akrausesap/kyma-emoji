...
var applicationLogUtils = require('../utils/application-logs-utils');
...
function onResponse(requests, requestStats, next) {
  ...
  if (outgoingResponse.statusCode < 100 || outgoingResponse.statusCode > 999){
    returnGateWayError(500, next, new VError('Invalid status code: %d returned from backend request to %s', outgoingResponse.statusCode, url.href));
...
function handleBackendError(incomingRequest, incommingResponse){
  if (incommingResponse.statusCode > 399 && incommingResponse.statusCode < 600){
    var errorMessage = incomingRequest.method + ' request to ' + incomingRequest.internalUrl.href + ' completed with status ' + incommingResponse.statusCode +
      ' ' + incommingResponse.statusMessage + ' check backend application logs';
    applicationLogUtils.logRequestError(incomingRequest, errorMessage);
  }
}
