...
module.exports = {
  ...
  exchangeToken: function(req, cb){
    ...
    tokenUtils.exchangeToken(req.session.user.token.accessToken, ownUAACredentials, req.destinationCredentials.uaa, function(err, accessToken) {
      if (err){
        return cb(err);
      }
      ...
      sessionExt.update(req.session, function(session) {
        ...
        session.user.businessServices[req.destinationCredentials.serviceName] = {
          ...
          scopes: tokenDecoded.granted_scopes || tokenDecoded.scope
        };
      });
      return cb(null);
    });
