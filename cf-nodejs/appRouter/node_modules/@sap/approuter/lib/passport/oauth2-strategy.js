...
OAuth2Strategy.prototype.authenticate = function (req, options) {
  ...
  if (!req.query || !req.query.code) {
    ...
    if (process.env.PRESERVE_FRAGMENT !== 'false') {
      ...
      cookieUtils.signCookie (req, originalUrl, function (err, signature) {
        ...
        var fragmentCookie = fragmentCookieName + '="+encodeURIComponent(location.hash)+";path=/";';
        var redirectCookie = redirectCookieName + '="+encodeURIComponent(location.href.split(\'#\')[0].split(location.host)[1])+";path=/";';
        var signatureCookie = signature + ';path=/";';
        if (process.env.COOKIES){
          try {
            var cookies = JSON.parse(process.env.COOKIES);
            if (cookies && cookies.SameSite === 'None'){
              var sameSiteString = ';Secure;SameSite=None;";';
              fragmentCookie  = fragmentCookie.slice(0, -2) + sameSiteString;
              redirectCookie  = redirectCookie.slice(0, -2) + sameSiteString;
              signatureCookie = signatureCookie.slice(0, -2) + sameSiteString;
            }
          } catch (error){
        ...
        req.res.end(
            ...
            '<script>' +
            'document.cookie="' + fragmentCookie +
            'document.cookie="' + redirectCookie +
            'document.cookie="signature=' + signatureCookie +
  ...
  if (process.env.PRESERVE_FRAGMENT !== 'false') {
    var cookies = req.headers.cookie && req.headers.cookie.split(/\s*;\s*/).reduce((acc,cur) => {
    ...
    var redirectCookie = querystring.unescape(cookies[redirectCookieName] && cookies[redirectCookieName][0]);
