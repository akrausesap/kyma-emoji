...
var cacheServiceDestinations = require('../middleware/service-destinations-middleware');
  ...
  handleTokenExchange(req,function (err) {
    ...
    try {
      ...
      if (req.headers && req.headers['sec-websocket-protocol']) {
        options.protocol = req.headers['sec-websocket-protocol'];
      }
      outgoing = new WebSocket(url, options);
    } catch (err) {
...
function getServiceToken(req){
  ...
  var token = null;
  if (req.session && req.session.user && serviceTag && businessServiceUtils.getGrantType(req.internalUrl.route.credentials) === 'user_token') {
